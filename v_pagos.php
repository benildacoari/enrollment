        ...
        echo "<div class='panel panel-primary'>";
          ...
          <div class='panel-body'>
            <table class='table table-striped table-bordered'>
              <thead><tr><th>ESPECIALIDAD</th><th>FECHA</th><th>N° BOLETA</th><th>DETALLE</th><th>MONTO</th><th></th><th colspan='2'></th></tr></thead><tr>";
              $query_p='SELECT * from matricula as ma INNER JOIN pagos as pa INNER JOIN especialidad as es 
              where ma.idmatricula=pa.idmatricula and ma.id_especialidad=es.id_especialidad and ma.iddat_pers='.$iddat.' ORDER BY pa.fecha_b DESC';
              ...
              while($dato=mysqli_fetch_array($resultado1)){
                ...
                <a href='del_pag.php?idp=".$dato['idpagos']."&idni=".$iddat."&idesp=".$idespec."&idm=".$idmt."'";?> 
  ...
  echo "<div class='row'>
    ...
    <div class='col-lg-8 col-md-10 col-sm-10 col-xs-10'>";
      ...
      if(isset($_REQUEST['n_pago'])){
        echo "<form action='v_rpago.php' method='POST' class='form-horizontal' role='form'>
          <div class='form-group'>
            <label class='col-lg-6 col-md-6 col-sm-6 col-xs-3 control-label'>Especialidad:</label>
            <div class='col-lg-4 col-md-4 col-sm-4 col-xs-6'>
              <input type='number' name='mat' VALUE=".$idmt." class='sr-only'>";

                $query="SELECT * FROM especialidad WHERE id_especialidad=".$idespec."";
                $resultado =mysqli_query($link,$query) or die("Problemas en el select:".mysqli_error($link));
                while($row = mysqli_fetch_array($resultado)){
                  echo "<input type='number' name='espec' VALUE=".$row['id_especialidad']." class='sr-only'>".$row['nombre_espec']."";
                }
            echo "</div>
          </div>
